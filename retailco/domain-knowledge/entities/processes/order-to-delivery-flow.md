...
### Backorder Path
If inventory allocation determines no stock is available (ATP check fails), the order is marked as `backordered` instead of proceeding to fulfillment. The customer sees no delivery date. This can happen incorrectly when upstream data is corrupted — e.g., a Change Data Capture pipeline tagging orders with wrong region metadata causes ATP to check inventory in the wrong region. See `backordered`, `change-data-capture`, `available-to-promise`.

### Known Failure Modes
- **False backorders via data replication error**: Orders backordered despite stock being available at distribution points. Root cause: misconfigured Change Data Capture pipeline tagged orders with wrong region, so ATP checked inventory in a region with no stock. Pattern: correct logic, corrupted data. See `backordered`, `change-data-capture`.
