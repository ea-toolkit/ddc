...
11. **Migration-induced database throttling via access level bug** — during RoutingPlatformV1 to V2 user migration, a bug in RoutingPlatformV1's search shipment page caused users with historically modified access levels to trigger expensive database queries. Shared database throttled, connection pools on order handler components exhausted. Picking Service's synchronous API calls backed up. User reported the issue before system detection. Rollback took over an hour; order handler pods scaled horizontally and servers restarted. Second access level incident on this platform. See `migration-traffic-amplification`.
